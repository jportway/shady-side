...
import com.jsuereth.gl.io.{withMemoryStack, VertexArrayObject, ShaderLoadingEnvironment, ActiveTextures}
import org.lwjgl.system.MemoryStack
import com.jsuereth.gl.mesh.parser.ObjFileParser
import com.jsuereth.gl.scene._
import com.jsuereth.gl.mesh._
import com.jsuereth.gl.texture.{Texture, Texture2D}
import com.jsuereth.gl.math._
import com.jsuereth.gl.io.VertexBufferObject
...
object Main {
  ...
  def run(): Unit = {
    ...
    System.out.println(DefaultShader.vertexShaderCode)
    System.out.println("--- Fragment Shader ---")
    System.out.println(DefaultShader.fragmentShaderCode)
    System.out.println("---  ---")
    try {
      init()
      loop()
  ...
  private def loop(): Unit = {
    ...
    scene = SimpleStaticSceneBuilder()
      ...
      .orientation(Quaternion.fromEuler(0f, 90f, 90f))
      .done()
      .add(mesh)
      .scale(scaleFactor, scaleFactor, scaleFactor)
      .pos(Vec3(5f, 2f, 4f))
      .orientation(Quaternion.fromEuler(90f, 0f, 0f))
      .done()
      .add(mesh)
      .scale(scaleFactor, scaleFactor, scaleFactor)
      .pos(Vec3(-5f, 4f, 4f))
      .orientation(Quaternion.fromEuler(0f, 0f, 90f))
      .done()
    ...
    System.out.println(s" world: ${DefaultShader.debugUniform("world")}")
    System.out.println(s" world.light: ${DefaultShader.debugUniform("world.light")}")
    System.out.println(s" world.eye: ${DefaultShader.debugUniform("world.eye")}")
