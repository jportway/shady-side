...
/** Returns the bytes required to store a given value in GLSL. Note: This does NOT work on dynamic data. It returns a staticly known size for a type,
...
/** Calculates the OpenGL enum for a type we'll pass into a VAO. Note: For types like vec2 or mat4x4, this will return the primitive type, as OpenGL
  * specifies these with primitive + size.
  */
...
/** Calculates the size for OpenGL vertex attribute calls. Essentially just how many of a give primitive are stored in a type.
  */
...
inline def isGLPrimitive[T]: Boolean = inline erasedValue[T] match {
  // TODO - Modern OpenGL types: GL_FIXED, GL_HALF_FLOAT,GL_INT_2_10_10_10_REV, GL_UNSIGNED_INT_2_10_10_10_REV, GL_UNSIGNED_INT_10F_11F_11F_REV
  case _: Boolean      => true
  case _: Byte         => true
  case _: Short        => true
  ...
  case _: Vec3[t]      => true
  case _: Vec4[t]      => true
  case _: Matrix4x4[t] => true
  case _: Matrix3x3[t] => true
  case _               => false
}
...
inline def vaoAttributes[T]: Array[VaoAttribute] = {
  val stride = sizeOf[T]
  inline if (isGLPrimitive[T]) {
    Array(attr[T](0, stride, 0))
  } else {
    inline erasedValue[T] match {
      case _: Tuple1[a] => Array(attr[a](0, stride, 0))
      // TODO - use a continuation approach to tease apart tuples, like we do in sizeOf.
      case _: (a, b)    => Array(attr[a](0, stride, 0), attr[b](1, stride, sizeOf[a]))
      case _: (a, b, c) => Array(attr[a](0, stride, 0), attr[b](1, stride, sizeOf[a]), attr[c](2, stride, sizeOf[a] + sizeOf[b]))
      case _: (a, b, c, d) =>
        Array(
          attr[a](0, stride, 0),
      ...
      case _: Product =>
        summonFrom { case m: Mirror.ProductOf[T] =>
          vaoAttributes[m.MirroredElemTypes]
        }
